...
if __name__ == "__main__":
    ...
    if args.resume:
        encoder.load_state_dict(torch.load('save_models/epoch17_cm0.5458_pretrain_encoder.pth'))
        regressor.load_state_dict(torch.load('save_models/epoch17_cm0.5458_pretrain_regressor.pth'))
        domain_predictor.load_state_dict(torch.load('save_models/epoch17_cm0.5458_pretrain_domain.pth'))
        args.epoch_reached = 18
        print('resuming from {}'.format(args.epoch_reached))
    ...
    for epoch in range(args.epoch_reached, args.epochs + 1):
        if epoch < args.epoch_stage_1:
            ...
            if epoch == args.epoch_stage_1 - 1:
                ...
                save_all_models('checkpoint_epoch{}_pretrain_cm{:.4f}'.format(epoch, cm), encoder, regressor,
                                domain_predictor)
        else:
            print('Unlearning')
            print('Epoch ', epoch, '/', args.epochs, flush=True)
            ...
            if epoch == args.epochs:
                ...
                print("Checkpoint, saving model")
                save_all_models('checkpoint_epoch{}_unlearning_cm{:.4f}'.format(epoch, cm), encoder, regressor, domain_predictor)
